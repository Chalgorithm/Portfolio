...
class Matrix:
    ...
    def __mul__(self,right):
        ...
        if type(right) == int:
            ...
            mat = []
            for y in range(0,right.rows):
                mat.append([0]*len(self.matrix[0]))
                
            print(Matrix(mat))
            print("rows:"+str(len(mat)))
            print("cols:"+str(len(mat[0])))
            #print("columns:"+str(len(mat[0])))
            for y in range(0,len(mat)):
                for x in range(0,len(mat)):
                    mat[x][y] = self.dotProduct(right,y,x)
            return Matrix(mat)
